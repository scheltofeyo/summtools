...
interface SidePanelProps {
  ...
  onToggle: () => void;
  activeNames: string[];
  removedNames: Array<{ name: string; timestamp: number }>;
  allWinners: Array<{ name: string; timestamp: number }>;
  onActiveNamesChange: (names: string[]) => void;
  onRemovedNamesChange: (names: Array<{ name: string; timestamp: number }>) => void;
}
...
export function SidePanel({
  ...
  const handleAddName = () => {
    if (inputValue.trim()) {
      // Split by comma, newline, or semicolon to allow multiple names
      const newNames = inputValue
        .split(/[,;\n]+/)
        .map(name => name.trim())
        .filter(name => name.length > 0)
        .filter(name => !activeNames.includes(name)); // Avoid duplicates
  ...
  return (
    <>
      {/* Toggle Button - Always Visible on Desktop, Hidden on Mobile when closed */}
      <motion.button
        onClick={onToggle}
