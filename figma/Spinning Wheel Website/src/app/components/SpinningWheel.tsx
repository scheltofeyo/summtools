...
export function SpinningWheel({ names, isSpinning, onSpinComplete, rotation }: SpinningWheelProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [dimensions, setDimensions] = useState({ width: 600, height: 600 });
  const previousIsSpinningRef = useRef(isSpinning);
  const [easingCurve, setEasingCurve] = useState<[number, number, number, number]>([0.33, 0, 0.15, 1]);
  ...
  useEffect(() => {
    ...
    if (isSpinning && !previousIsSpinningRef.current) {
      // Create curve for: DRAMATIC final crawl
      // For a very slow ending, both y1 and y2 should be close to 1 (creates flat curve at end)
      // This makes the animation cover most distance quickly, then crawl through final degrees
      const x1 = 0.25 + Math.random() * 0.1; // 0.25 to 0.35
      const y1 = 0.9 + Math.random() * 0.08; // 0.9 to 0.98 - reach almost complete early
      const x2 = 0.65 + Math.random() * 0.1; // 0.65 to 0.75
      const y2 = 0.98 + Math.random() * 0.015; // 0.98 to 0.995 - creates very flat curve at end (slow crawl)
    ...
    if (!isSpinning && previousIsSpinningRef.current) {
      // Calculate and immediately report winner when spinning stops
      const normalizedRotation = ((rotation % 360) + 360) % 360;
      const segmentAngle = 360 / names.length;
      // The pointer is at the top (270 degrees or -90 degrees from right)
      // We need to find which segment is at 270 degrees after rotation
