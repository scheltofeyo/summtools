...
// Match the colors from SpinningWheel.tsx
const WHEEL_COLORS = [
...
// Convert hex to a lighter version for gradient end
...
export function WinnerOverlay({ winner, winnerIndex, isVisible }: WinnerOverlayProps) {
  ...
  return (
    <AnimatePresence>
      {isVisible && winner && (
        ...
        >
          {/* Background Overlay - Semi-transparent, non-blocking */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            {[...Array(30)].map((_, i) => {
              // Generate random spread positions from center
              const angle = (Math.random() * 360 * Math.PI) / 180;
              const distance = 200 + Math.random() * 400; // pixels from center
              const finalX = Math.cos(angle) * distance;
              const finalY = Math.sin(angle) * distance - 100; // Bias upward
          ...
          {/* Winner Content - Positioned to not block bottom controls */}
          <motion.div
